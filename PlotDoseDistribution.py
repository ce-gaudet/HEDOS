...
def plot_dose_distribution(blood_dose_total, dose_contributions, plan_name, run_number, mean_blood_dose=None):
    ...
    for organ, dose in dose_contributions.items():
        ax2.hist(dose, bins=bins2[2:], histtype='step', linewidth=1.5, alpha=0.7, label=organ)
...
class IndexTracker(object):
    def __init__(self, ax, X, Y, plot_slice, cmap_ref='Greys', cmap='Greys_r'):
        ...
        # compute global min/max over the whole dose volume
        self.vmin = np.min(self.X)
        self.vmax = np.max(self.X)

        self.im1 = ax.imshow(
            self.X[:, :, self.plot_slice],
            cmap=cmap_ref,
            vmin=self.vmin,
            vmax=self.vmax
        )
